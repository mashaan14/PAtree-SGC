...
class BinaryTree(object):
    ...
    def construct_tree(self, tree, index):
        nTry = 3 #100 #1000
        whichProjection = 'PCA' # '2008_Dasgupta' # '2019_Yan' #  'proposed' # 'PCA'
        ...
        # SplitPoint = random.uniform(np.quantile(X_proj[:,SplitDimension], 0.25, axis=0),
        #                             np.quantile(X_proj[:,SplitDimension], 0.75, axis=0))
        SplitPoint = random.uniform(np.quantile(X_proj[:, SplitDimension], 0.5, axis=0),
                                    np.quantile(X_proj[:, SplitDimension], 0.5, axis=0))
